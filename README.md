# VIA (Vision Impairment App) — Accessible Audio Player
...
* **Gesture & Audio Navigation:** Users navigate via taps, long-presses, and dual-holds. Every action triggers haptic feedback, instantaneous UI jingles, and an audio confirmation in Hebrew via Azure's Neural TTS.
...
* **Hidden Admin Controls:** To keep the interface completely uncluttered for the primary user, all configuration and maintenance tools are hidden behind a secure, delta-time multi-tap sequence on the title button.
...
* **Audio Engine (Media3 & ExoPlayer):** Handles robust streaming of remote Dropbox URLs via a bound background `PlaybackService`. It manages audio focus (pausing during phone calls) and custom WakeLocks to prevent the CPU from sleeping while streaming.
* **API Management (Retrofit & Coroutines):** Retrofit handles all secure communications with the Dropbox API (fetching temporary streaming links, syncing directory trees) and the Azure Cognitive Services API (generating Hebrew TTS). All network requests are handled safely off the main thread using Kotlin Coroutines, utilizing custom mutex locks and state flags to prevent race conditions from multi-touch spam.
...
* **Zero-Latency UI Feedback:** To prevent UI thread freezing and audio engine startup lag, short UI sound effects are pre-loaded directly into RAM using Android's native `SoundPool` API, synchronized precisely with TTS callbacks.
...
* **Media:** AndroidX Media3 (ExoPlayer, MediaSession, MediaController), SoundPool
